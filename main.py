...
HISTORY_MAX_LEN = 500
history: deque = deque(maxlen=HISTORY_MAX_LEN)
...
if not OPENAI_API_KEY:
    # Puedes cambiar esto a un warning/log si prefieres no romper en import
    raise RuntimeError(
...
class JudgeRequest(BaseModel):
    ...
    answer_a: str
    answer_b: str
    judge_model: Optional[ModelCfg] = None  # if not provided, use default or heuristic
...
async def call_openai_via_aisuite(model: str, prompt: str, temperature: float, max_tokens: int) -> str:
    """
    Uses aisuite if disponible. Debe estar configurado con OPENAI_API_KEY en el entorno.
    """
...
async def call_openai_sdk(model: str, prompt: str, temperature: float, max_tokens: int) -> str:
    """
    Llamada real a OpenAI. Sin mocks.
    Ejecutamos la llamada bloqueante en un hilo usando asyncio.to_thread.
    """
...
def heuristic_judge(task_type: str, prompt: str, a: str, b: str) -> Dict[str, Any]:
    def score(ans: str) -> Dict[str, float]:
        ...
        # Nuevo cálculo de task fit
        overlap_ratio = len(set(prompt.lower().split()) & set((ans or "").lower().split())) / max(1, len(prompt.split()))
        fit = 1 + 4 * overlap_ratio  # continuo entre 1–5
...
async def model_judge(judge_model: str, task_type: str, prompt: str, a: str, b: str) -> Dict[str, Any]:
    ...
    try:
        ...
        out = {
            ...
            "coverage_A": float(data.get("coverage_A", 0.0)),
            "coverage_B": float(data.get("coverage_B", 0.0)),
            "missing_A": _safe_list(data.get("missing_A", [])),
            "missing_B": _safe_list(data.get("missing_B", [])),
            "winner": data.get("winner", "Tie"),
            "rationale_short": str(data.get("rationale_short", ""))[:800],
        }
        return out
    except Exception:
        return heuristic_judge(task_type, prompt, a, b)
...
async def compare(req: CompareRequest):
    ...
    async def run_one(cfg: ModelCfg) -> ModelRun:
        ...
        try:
            ...
            # devolvemos error limpio al front si algo falla
...
async def judge(req: JudgeRequest):
    try:
        judge_model = (req.judge_model.model if req.judge_model else "gpt-4o-mini")
        data = await model_judge(judge_model, req.task_type, req.prompt, req.answer_a, req.answer_b)
    except Exception:
